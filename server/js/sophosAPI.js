...
async function runDetectionQuery() {
  if (!access_token) {
    Authenticate();
  }
  if (!tenantID) {
  ...
  try {
    const res = await fetch(
      `https://api-${dataRegion}.central.sophos.com/detections/v1/queries/detections`,
      {
        method: "POST",
        headers: {
...
async function getDetectionQueryStatus(queryId) {
  if (!access_token) {
    Authenticate();
  }
  if (!tenantID) {
  ...
  try {
    const res = await fetch(
      `https://api-${dataRegion}.central.sophos.com/detections/v1/queries/detections/${queryId}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${access_token}`,
      ...
      if (response.status === 401) {
        Authenticate();
        getDetectionQueryStatus(queryId);
      }
      if (!response.ok) {
        throw new Error("Network response was not ok " + response.statusText);
      }
...
async function getDetectionQueryResults(queryId) {
  if (!access_token) {
    Authenticate();
  }
  if (!tenantID) {
  ...
  try {
    const res = await fetch(
      `https://api-${dataRegion}.central.sophos.com/detections/v1/queries/detections/${queryId}/results`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${access_token}`,
      ...
      if (response.status === 401) {
        Authenticate();
        getDetectionQueryResults(queryId);
      }
      if (!response.ok) {
        throw new Error("Network response was not ok " + response.statusText);
      }
...
async function getDetections() {
  ...
  return runDetectionsQuery().then((response) => {
    ...
    try {
      return fetch(
        `https://api-${dataRegion}.central.sophos.com/detections/v1/queries/detections/${detectionsId}/results?page=1&pageSize=1000`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${access_token}`,
