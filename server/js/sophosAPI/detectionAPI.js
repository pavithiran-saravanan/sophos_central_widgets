...
router.get("/status", async (req, res) => {
  try {
    const data = await getDetectionQueryStatus();
...
async function getDetectionQueryStatus() {
  if (!runID) {
    await setRunID();
  }
  const res = await fetch(
    `https://api-${authProp.dataRegion}.central.sophos.com/detections/v1/queries/detections/${runID}`,
    {
      method: "GET",
      headers: {
        Authorization: `Bearer ${authProp.access_token}`,
