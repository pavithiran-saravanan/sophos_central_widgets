...
async function setData() {
  ...
  const detections = await fetch("/detections/results").then(
  ...
  detections.items.forEach((detection) => {
    if (detection.mitreAttacks && detection.mitreAttacks.length > 0) {
      detection.mitreAttacks.forEach((attack) => {
        ...
        if (!tacticTechniqueCounter[tactic.name])
          tacticTechniqueCounter[tactic.name] = {};
        if (!tacticTechniqueSeverity[tactic.name])
          tacticTechniqueSeverity[tactic.name] = {};
        tactic.techniques.forEach((technique) => {
          if (!techniquesInfo[technique.id])
            techniquesInfo[technique.id] = technique;
          if (!tacticTechniqueCounter[tactic.name][technique.id])
            tacticTechniqueCounter[tactic.name][technique.id] = 1;
          else tacticTechniqueCounter[tactic.name][technique.id]++;
          if (!tacticTechniqueSeverity[tactic.name][technique.id])
            tacticTechniqueSeverity[tactic.name][technique.id] =
              detection.severity;
        });
      });
    }
  ...
  const detectionsHeatMapContainer = document.querySelector(
    "#detectionsHeatMapWidget"
  );
  ...
  Object.entries(tacticTechniqueCounter).forEach((entry) => {
    ...
    let alertsCount = techniqueCounts.reduce(
    ...
    techniquIds.forEach((techniqueId) => {
      attackDiv.append(
        getTechniqueDiv(
          techniqueId,
          techniquesInfo[techniqueId].name,
          tacticTechniqueSeverity[entry[0]][techniqueId]
        )
      );
    });
